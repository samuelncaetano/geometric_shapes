...
class Triangle(GeometricShape):
    def __init__(self, ponto1, ponto2, ponto3):
        if self.formar_triangulo(ponto1, ponto2, ponto3):
            ...
            raise ValueError(
                "Os pontos fornecidos não formam um triângulo válido.")
    ...
    def calcular_centro(self):
        x = (self.__ponto1.get_x() + self.__ponto2.get_x() +
             self.__ponto3.get_x()) / 3
        y = (self.__ponto1.get_y() + self.__ponto2.get_y() +
             self.__ponto3.get_y()) / 3
        centro_x = round(x, 1)
        centro_y = round(y, 1)
    ...
    def mover(self, novo_ponto):
        ...
        self.__ponto1 = Point(self.__ponto1.get_x() + dx,
                              self.__ponto1.get_y() + dy)
        self.__ponto2 = Point(self.__ponto2.get_x() + dx,
                              self.__ponto2.get_y() + dy)
        self.__ponto3 = Point(self.__ponto3.get_x() + dx,
                              self.__ponto3.get_y() + dy)
